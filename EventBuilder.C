...
///////////////////////////////////
  ...
  int i=0;
  int j=1;
  long long int t_gate=1000; // in ns.
  ...
  while(i < tree->GetEntries()){
    tree->GetEntry(i);
    if(ext->GetValue()==1 && fe==0){fe=1; ext_time=t->GetValue(); em=1;} //// Give external flag
    if(ext->GetValue()==0){
      time_ref0=t->GetValue();
      timestp_0.push_back(t->GetValue());
      det0.push_back(det->GetValue());
      ...
      s0[int(det->GetValue())][int(side->GetValue())] = ch->GetValue() + 128*asic->GetValue(); //fills first strip anchor
    ...
    for(j=i+1;j<i+100;j++){
      ...
      time_ref1=t->GetValue();
        if(abs(time_ref0-time_ref1)>t_gate){ //check coincidence
          ...
          if(ext->GetValue()==1){fe=1; ext_time=t->GetValue(); em++;} //// Give external flag
          if(ext->GetValue()==0){
